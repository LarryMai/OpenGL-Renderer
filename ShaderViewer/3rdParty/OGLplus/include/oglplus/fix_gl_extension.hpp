/**
 *  .file oglplus/config/fix_gl_extension.hpp
 *  .brief Helper file for adjusting GL extension PP symbols
 *
 *  @author Matus Chochlik
 *
 *  Copyright 2013-2015 Matus Chochlik. Distributed under the Boost
 *  Software License, Version 1.0. (See accompanying file
 *  LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)
 *
 *  Automatically generated file.
 */
#ifndef OGLPLUS_FIX_GL_EXTENSION_1312210942_HPP
#define OGLPLUS_FIX_GL_EXTENSION_1312210942_HPP
#ifdef GL_ARB_shader_subroutine
#undef GL_ARB_shader_subroutine
#endif //GL_ARB_shader_subroutine
#ifdef GL_ARB_shading_language_include
#undef GL_ARB_shading_language_include
#endif //GL_ARB_shading_language_include
#ifdef GL_ARB_gpu_shader_fp64
#undef GL_ARB_gpu_shader_fp64
#endif //GL_ARB_gpu_shader_fp64
#ifdef GL_ARB_direct_state_access
#undef GL_ARB_direct_state_access
#endif //GL_ARB_direct_state_access
#ifdef GL_EXT_direct_state_access
#undef GL_EXT_direct_state_access
#endif //GL_EXT_direct_state_access
#ifdef GL_NV_path_rendering
#undef GL_NV_path_rendering
#endif //GL_NV_path_rendering
#ifdef GL_NV_copy_image
#undef GL_NV_copy_image
#endif //GL_NV_copy_image
#endif //include guard
